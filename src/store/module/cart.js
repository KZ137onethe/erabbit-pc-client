...
const CartModule = {
  ...
  getters: {
    ...
    isAllSelected(state, getters) {
      return (
        getters.validateList.length && getters.validateList.length === getters.selectedList.length
      )
    },
  },
  mutations: {
  ...
  actions: {
    ...
    modifyAllCart({ state, commit }, payload) {
    ...
    // 删除选中的购物车商品（批量）
    removeCartSelected({ _getters, commit }, dataSource) {
      console.log("批量删除商品中...")
      return new Promise((resolve) => {
        if (isLogin.value) {
          const ids = dataSource.map((item) => item.skuId)
    ...
    updateCartGoodsSku({ state, commit }, payload) {
