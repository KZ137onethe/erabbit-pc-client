<template>
	<!-- ? title 属性： 当鼠标移动到带有 title 属性的元素上时，提示文本将作为工具提示(tooltip)显示出来 -->
		...
		<div class="item" v-for="item in filterData.saleProperties" :key="item.id">
			...
			<a-row type="flex" justify="start" align="middle" :wrap="true">
				<a-col v-for="prop in item.properties" :key="prop.id">
					<a href="javascript:;" @click="attrSelected(item, prop.id)" :class="{ active: onlySelected(prop.id, item.selected) }" :title="prop.name">
						{{ prop.name }}
					</a>
				</a-col>
			</a-row>
