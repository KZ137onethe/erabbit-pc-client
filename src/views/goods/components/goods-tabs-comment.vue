  ...
  <div v-if="appraisalInfo" class="goods-tabs-comment">
    <header>
      <div class="comment-data">
        <div class="buy">
          <span>{{ appraisalInfo.salesCount }}</span><span>人购买</span>
        </div>
        <div class="good-rate">
          <span>{{ appraisalInfo.praisePercent }}</span><span>好评率</span>
        </div>
      </div>
      <div class="comment-tags">
        <a-row type="flex" :wrap="false">
          ...
          <a-col flex="auto" class="tags">
            <a v-for="(item, index) in appraisalInfo.tags" :key="item"
               href="javascript:;" :class="{ active: currentTagIndex === index}"
    ...
    <a-tabs v-model:activeKey="commentKey">
      ...
      <a-tab-pane v-for="nowKey in Object.keys(tabsKey)" :key="nowKey" :tab="tabsKey[nowKey].message">
        <template v-if="commentData">
          <a-comment v-for="data in commentData[pagination.currentPage - 1]" :key="data.id">
            ...
            <template #content>
              ...
              <!-- 预览图片 -->
              <GoodsTabsCommentPreviewImg v-if="data.pictures.length" :picture="data.pictures"/>
            </template>
          </a-comment>
        </template>
      </a-tab-pane>
