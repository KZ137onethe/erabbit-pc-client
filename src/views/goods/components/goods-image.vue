...
export default {
  ...
  setup(props) {
    ...
    watch([elX, elY, isOutside], () => {
      // 遮罩层的处理
      if (elX.value >= 100 && elX.value <= 300) {
        maskPos.left = elX.value - 100
      } else if (elX.value < 100) {
        maskPos.left = 0
      } else {
        maskPos.left = 200
      }

      if (elY.value >= 100 && elY.value <= 300) {
        maskPos.top = elY.value - 100
      } else if (elY.value < 100) {
        maskPos.top = 0
      } else {
        maskPos.top = 200
      }
      ...
      enlargeStyle.backgroundPositionX = `${-2 * parseInt(maskPos.left, 10)}px`
      enlargeStyle.backgroundPositionY = `${-2 * parseInt(maskPos.top, 10)}px`
    })
    return {
      pictures,
      imgIndex,
