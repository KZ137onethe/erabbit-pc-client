<template>
  <div v-if="Object.keys(goods).length" class="goods-sku">
    <div v-for="(item, index) in goods.specs" :key="item.id" class="item">
      <header>{{ item.name }}:</header>
      <a-row type="flex" justify="start" :wrap="true">
        <a-col v-for="val in item.values" :key="val.name" flex="50px">
...
export default {
  ...
  setup(props, { emit }) {
    const { goods, skuId } = reactive(props)
    // value对象 => 规格表单
