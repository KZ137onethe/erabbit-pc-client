<template>
	<a-layout-content class="order_detail-content">
		<!-- 步骤条 -->
		<a-steps class="detail-content--steps" v-model:current="current" status="wait">
		...
		<div class="detail-content--address">
			...
			<div class="card">
				<a-descriptions :column="1">
					...
					<a-descriptions-item label="收货地址">{{ orderData.receiverAddress }}</a-descriptions-item>
				</a-descriptions>
			</div>
		</div>
		<!-- 支付方式和送货时间 -->
		<div class="detail-content--pay">
			...
			<div class="card">
				<a-descriptions :column="1">
					...
					<a-descriptions-item label="送货时间">双休日、假日送货（适合于家庭地址）</a-descriptions-item>
				</a-descriptions>
			</div>
		</div>
		<!-- 发票信息 -->
...
export default {
	...
	setup(props) {
		...
		const current = computed(() => (orderData.value.orderState === 6 ? 0 : orderData.value.orderState))
