<template>
  <div class="shopping-cart">
    <div class="container">
      ...
      <!-- 购物车面板 -->
      <a-table
        v-model:data-source="currentData"
        sticky
        :columns="columns"
        :pagination="false"
        align="right"
        ...
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'Selected'">
            <a-checkbox
              ...
              @change="modifySelected($event, record)"
            />
          </template>
          <template v-else-if="column.key === 'Name'">
            <div class="preview">
              ...
              <div class="describe">
                ...
                <cart-sku
                  v-model:attrsText="record.attrsText"
                  :sku-id="record.skuId"
                  @amend="updateCartSku($event, record.skuId)"
                ></cart-sku>
              </div>
            </div>
          </template>
          ...
          <template v-else-if="column.key === 'Action'">
            <span>
              ...
              <a-button type="link" style="padding: 3px 0" @click="deleteCartGoods(record)">
                删除
              </a-button>
              <a-divider type="vertical" />
          ...
          <a-table-summary :fixed="fixedTop ? 'top' : 'bottom'">
            <a-table-summary-row>
              <a-table-summary-cell :index="0" :col-span="1">
                <a-checkbox
                  v-model:checked="columns[0].checked"
                  @change="modifyAllSelected($event, currentData)"
                >
                  全选
                </a-checkbox>
...
export default defineComponent({
  ...
  setup() {
    ...
    const modifyAllSelected = (event, dataSource) => {
      modifyAllCart({ selected: event.target.checked, dataSource })
      for (const [key, values] of Object.entries(columns.value)) {
    ...
    const checkBtn = computed(() => !(validateList.value.length && selectedList.value.length))
