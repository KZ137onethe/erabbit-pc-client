<template>
  <div ref="target" class="xtx-city">
    ...
    <div v-if="active" class="option">
      <a-row v-if="!loading" :wrap="true" align="middle" :justify="start">
        <a-col
          v-for="item in currentCitySet"
          :key="item.code"
          :span="4"
  ...
  setup(props, { emit }) {
    ...
    const currentCitySet = computed(() => {
      ...
      if (selectedArea.provinceCode) {
        arrSet.value = arrSet.value.find(
          (item) => item.code === selectedArea.provinceCode,
        )?.areaList
      }
      // 转入县一级
